...
class Solution {
  public:
    vector<int> findSubstring(string s, vector<string> &words) {
        vi res;
        umap<string, int> map;
        int length = sz(words[0]);

        map.clear();
        for (const string &word : words)
            map[word]++;

        loop(offset, 0, length) {
            int size = 0;
...
int main() {
    ...
    auto res = obj.findSubstring("barfoothefoobarman", w);
    print(res);
    w = {"word", "good", "best", "word"};
    res = obj.findSubstring("wordgoodgoodgoodbestword", w);
    print(res);
    w = {"bar", "foo", "the"};
    res = obj.findSubstring("barfoofoobarthefoobarman", w);
    print(res);
    return 0;
}
