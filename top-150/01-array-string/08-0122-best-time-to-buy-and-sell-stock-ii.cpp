...
class Solution {
  private:
    int solve(int idx, int buy, vi &prices, vvi &memo) {
        ...
        int res = 0;
        if (buy) {
            updateMax(res, -prices[idx] + solve(idx + 1, 0, prices, memo));
            updateMax(res, solve(idx + 1, 1, prices, memo));
        } else {
            updateMax(res, prices[idx] + solve(idx + 1, 1, prices, memo));
            updateMax(res, solve(idx + 1, 0, prices, memo));
        }
    ...
    int maxProfit(vector<int> &prices) {
        ...
        revloop(idx, n - 1, 0) {
            loop(buy, 0, 2) {
                ...
                if (buy) {
                    updateMax(res, -prices[idx] + dp[idx + 1][0]);
    ...
    int maxProfit(vector<int> &prices) {
        ...
        revloop(idx, n - 1, 0) {
            loop(buy, 0, 2) {
                ...
                if (buy) {
                    updateMax(res, -prices[idx] + next[0]);
