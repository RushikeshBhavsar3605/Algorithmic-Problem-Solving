...
class Solution {
  public:
    int maxFixedPoints(vector<int> &nums) {
        ...
        vvi buckets(maxVal + 1);

        loop(i, 0, n)
            if (nums[i] <= i)
                buckets[nums[i]].push_back(i - nums[i]);
        ...
        loop(i, 0, maxVal + 1) {
            ...
            iterate(slack, buckets[i]) {
                int best = 1 + seg.query(0, slack, 0, 0, n - 1);
                pending.push_back({slack, best});
                res = max(res, best);
            }

            iterate(p, pending)
                seg.update(p.first, p.second, 0, 0, n - 1);
        }
...
int main() {
    ...
    Solution obj;
    vi v = {0, 2, 1};
    auto res = obj.maxFixedPoints(v);
    print(res);
    v = {3, 1, 2};
    res = obj.maxFixedPoints(v);
    print(res);
    v = {1, 0, 1, 2};
    res = obj.maxFixedPoints(v);
    print(res);
...
/*
Problem: Maximize Fixed Points After Deletions
*/