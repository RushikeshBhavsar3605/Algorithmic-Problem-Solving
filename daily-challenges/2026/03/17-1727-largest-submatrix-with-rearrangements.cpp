...
/* Approach 1 (Precompute): */
class Solution {
  public:
    int largestSubmatrix(vector<vector<int>> &matrix) {
        int n = sz(matrix), m = sz(matrix[0]);
        int res = 0;
        vec<int> prevHeights(m, 0);

        loop(r, 0, n) {
            vec<int> heights = matrix[r];
            loop(c, 0, m)
                if (heights[c] != 0)
...
int main() {
    Solution obj;
    vec<vec<int>> v = {{0, 0, 1}, {1, 1, 1}, {1, 0, 1}};
    auto res = obj.largestSubmatrix(v);
    print(res);
    v = {{1, 0, 1, 0, 1}};
    res = obj.largestSubmatrix(v);
    print(res);
    v = {{1, 1, 0}, {1, 0, 1}};
    res = obj.largestSubmatrix(v);
    print(res);
