...
class Solution {
  private:
    int solve(int i, vi &nums, int target, vi &memo) {
        int n = sz(nums);
        ...
        loop(j, i + 1, n)
            if (abs(nums[j] - nums[i]) <= target) {
                int next = solve(j, nums, target, memo);
  ...
  public:
    int maximumJumps(vector<int> &nums, int target) {
        int n = sz(nums);
        vi memo(n, -1);
        int res = solve(0, nums, target, memo);
...
int main() {
    ...
    Solution obj;
    vi v = {1, 3, 6, 4, 1, 2};
    auto res = obj.maximumJumps(v, 2);
    print(res);
    v = {1, 3, 6, 4, 1, 2};
    res = obj.maximumJumps(v, 3);
    print(res);
    v = {1, 3, 6, 4, 1, 2};
    res = obj.maximumJumps(v, 0);
    print(res);
    return 0;
}
